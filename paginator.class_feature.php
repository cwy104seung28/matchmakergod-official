	...
	function paginate()
	{
		...
		if($this->num_pages > 6)
		{
			...
			for($i=1;$i<=$this->num_pages;$i++)
			{
				...
				{
					$this->return .= ($i == $this->current_page And $_GET['page'] != 'All') ? "<a title=\"Go to page $i of $this->num_pages\" class=\"relative -top-1 group current\" href=\"#\">$i</a> ":"<a class=\"paginate\" title=\"Go to page $i of $this->num_pages\" href=\"$_SERVER[PHP_SELF]?page=$i&ipp=$this->items_per_page$this->querystring\">$i</a> ";
				}
		...
		else
		{
			for($i=1;$i<=$this->num_pages;$i++)
			{
				$this->return .= ($i == $this->current_page) ? "<a class=\"relative -top-1 group current\" href=\"javascript:;\">
				<span class='font-light text-sm text-amber px-5'>$i</span>
				<span class='absolute inset-x-0 -bottom-[9px] h-[3px] bg-green opacity-100'></span>
				<span class='absolute inset-x-0 -bottom-[9px] h-[3px] bg-green transition-all duration-300 opacity-0 group-hover:opacity-100
								'></span></a> ":"<a class=\"relative -top-1 group paginate\" href=\"$_SERVER[PHP_SELF]?page=$i&ipp=$this->items_per_page$this->querystring\">
				<span class='font-light text-sm text-amber px-5'>$i</span>
				<span class='absolute inset-x-0 -bottom-[9px] h-[3px] bg-green opacity-25'></span>
				<span class='absolute inset-x-0 -bottom-[9px] h-[3px] bg-green transition-all duration-300 opacity-0 group-hover:opacity-100
								'></span></a> ";
			}
